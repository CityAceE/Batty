...
; Буфер для сохранения объектов с тенью и фоном из экранного буфера (не превышает $200 байт). 
...
; Если поменять адрес буфера, то лезут артефакты - не все адреса в процедурах расчёта ещё учтены
; attr_buff EQU $D700
attr_buff EQU $D700 ;- $100
...
;-----------------------------------------
; Восстановление фона под выбитым кирпичом
LAFFC_47:
  LD (LB3E2+$01),DE
  PUSH DE
  LD A,$20
...
;-----------------------------------------------
; Восстановление линии фона за выбитым кирпичом  
  LD A,(IX+$03)
  CP $08
  JR Z,LAFFC_51
  CP $0A
...
LAFFC_50:
  LDI		
  LDI		; Рисование линии за выбитым кирпичом
...
;-----------------------------------
; Назначение этого участка непонятно
LAFFC_56:
  LD DE,(LB28F+$01)
  POP HL
  LD B,H
...
;-----------------------------------------------
; Обработка соседнего кирпича над выбитым кирпичом
LAFFC_64:
  SET 0,L
  LD A,(LB292+$01)
  BIT 2,A
